...
export default function Hero() {
  ...
  const images = t("hero.images", { returnObjects: true }) as { subtitle: string; title: string; desc: string }[];
      ...
      <div style={{ position: "relative" }}>
        ...
        <div className="mt-5 flex justify-between items-end">
          ...
          <FadeIn delay={0.2}>
            <a
              href="#contact"
              className="block border border-[#1d454c] text-gray-500 text-xs uppercase tracking-[0.2em] px-8 py-3 hover:bg-[#1d454c] hover:text-[#eff3d4] transition-all duration-300 w-fit"
            >
              {t("hero.cta")}
            </a>
          </FadeIn>
      ...
      <div ref={containerRef} className="mt-10 h-[350vh]">
        <div className="sticky top-0 h-screen overflow-hidden"
          ...
          <motion.div className="absolute overflow-hidden"
            style={{ top: edgeGap, right: edgeGap, bottom: edgeGap, left: edgeGap,
              borderRadius, opacity: opacity1, backgroundImage: `url(${img1})`,
              backgroundSize: "cover", backgroundPosition: "center" }}>
            <div className="absolute inset-0 bg-linear-to-t from-black/65 via-black/15 to-transparent" />
            <div className="absolute bottom-12 left-12">
              <p className="text-white/55 text-xs uppercase tracking-[0.2em] mb-3">{images[0]?.subtitle}</p>
              <h2 className="text-white text-3xl font-light leading-snug">{images[0]?.title}</h2>
              <p className="mt-3 text-white/70 text-sm max-w-sm">{images[0]?.desc}</p>
            </div>
          </motion.div>
          ...
          <motion.div className="absolute overflow-hidden"
            style={{ top: edgeGap, right: edgeGap, bottom: edgeGap, left: edgeGap,
              borderRadius, opacity: opacity2, backgroundImage: `url(${img2})`,
              backgroundSize: "cover", backgroundPosition: "center" }}>
            <div className="absolute inset-0 bg-linear-to-t from-black/65 via-black/15 to-transparent" />
            <div className="absolute bottom-12 left-12">
              <p className="text-white/55 text-xs uppercase tracking-[0.2em] mb-3">{images[1]?.subtitle}</p>
              <h2 className="text-white text-3xl font-light leading-snug">{images[1]?.title}</h2>
              <p className="mt-3 text-white/70 text-sm max-w-sm">{images[1]?.desc}</p>
            </div>
          </motion.div>
          ...
          <motion.div className="absolute overflow-hidden"
            style={{ top: edgeGap, right: edgeGap, bottom: edgeGap, left: edgeGap,
              borderRadius, opacity: opacity3,
              background: "linear-gradient(135deg, #0f2a2e 0%, #1d454c 40%, #152f35 70%, #0a1d21 100%)" }}>
            ...
            {[
              ...
              <img key={i} src={coris2} alt="" aria-hidden="true"
                style={{ position: "absolute", top: c.top, left: c.left, width: c.size, height: c.size,
                  opacity: c.opacity, transform: `rotate(${c.rotate}deg)`, objectFit: "contain",
                  pointerEvents: "none", userSelect: "none", filter: "brightness(2)" }}
              />
            ))}
            <div className="absolute bottom-12 left-12">
              <p className="text-white/55 text-xs uppercase tracking-[0.2em] mb-3">{images[2]?.subtitle}</p>
              <h2 className="text-white text-3xl font-light leading-snug">{images[2]?.title}</h2>
              <p className="mt-3 text-white/70 text-sm max-w-sm">{images[2]?.desc}</p>
            </div>
          </motion.div>
        </div>
      </div>
