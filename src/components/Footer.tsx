...
export default function Footer() {
  ...
  const expertises = t("footer.expertises", { returnObjects: true }) as string[];
  ...
  return (
    <footer className="bg-[#1d454c] text-white">
      <div className="maxwidth mx-auto px-6 pt-24 pb-16 border-b border-white/10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-10">
          ...
          >
            ...
            <a href="#contact"
              className="border border-white/30 text-white/80 text-xs uppercase tracking-[0.2em] px-8 py-3 hover:bg-[#eff3d4] hover:text-[#1d454c] transition-all duration-300 w-fit">
              {t("footer.contactBtn")}
            </a>
          </motion.div>
        </div>
      ...
      <div className="maxwidth mx-auto px-6 py-16 grid grid-cols-2 md:grid-cols-[2fr_1fr_1fr_1fr] gap-12 border-b border-white/10">
        ...
        <FadeIn delay={0.1}>
          <p className="text-xs uppercase tracking-[0.2em] text-white/30 mb-5">{t("footer.navTitle")}</p>
          <ul className="space-y-3">
            {nav.map((link) => (
              <li key={link.label}>
                <a href={link.href} className="text-sm text-white/55 hover:text-white transition-colors duration-200">
                  {link.label}
                </a>
              </li>
            ))}
        ...
        <FadeIn delay={0.15}>
          <p className="text-xs uppercase tracking-[0.2em] text-white/30 mb-5">{t("footer.expertisesTitle")}</p>
          <ul className="space-y-3">
            {expertises.map((s) => (
        ...
        <FadeIn delay={0.2}>
          <p className="text-xs uppercase tracking-[0.2em] text-white/30 mb-5">{t("footer.contactTitle")}</p>
          <div className="space-y-3">
            <a href="mailto:contact@marabu.services"
              className="block text-sm text-white/55 hover:text-white transition-colors duration-200">
              contact@marabu.services
            </a>
            <a href="tel:+22507207770000"
              className="block text-sm text-white/55 hover:text-white transition-colors duration-200">
              +225 07 20 77 70 00
            </a>
          </div>
