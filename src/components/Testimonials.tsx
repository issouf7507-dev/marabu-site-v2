...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="py-24 bg-[#eff3d4]"
          ...
          {items.map((t_, i) => (
            <FadeIn key={i} delay={i * 0.08}>
              <div className="bg-white rounded-2xl p-8 flex flex-col gap-6 h-full">
                <span className="text-6xl font-serif leading-none text-[#538253] select-none">"</span>
                <p className="text-gray-500 leading-relaxed text-base flex-1">{t_.quote}</p>
                <div className="flex items-center gap-4 pt-2 border-t border-gray-100">
                  <div className="w-10 h-10 rounded-full bg-[#538253]/10 flex items-center justify-center shrink-0">
                    <span className="text-[#538253] font-semibold text-sm">{t_.name.charAt(0)}</span>
                  </div>
                  <div>
                    <p className="font-semibold text-gray-900 text-sm">{t_.name}</p>
                    <p className="text-gray-500 text-xs">{t_.role} · {t_.company}</p>
                  </div>
                </div>
              </div>
            </FadeIn>
