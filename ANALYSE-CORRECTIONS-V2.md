...
> « Du conseil stratégique à l'intermédiation internationale, Marabu Services accompagne les entreprises et les institutions dans leur transformation, leur rayonnement et le renforcement de leur influence. »
...
- **SEO :** `src/components/Seo.tsx` existe déjà (title, meta, Open Graph) + `public/sitemap.xml`, `robots.txt`, `og-image.jpg` présents. → vérifier que titre/description/OG correspondent bien à Marabu Services.
